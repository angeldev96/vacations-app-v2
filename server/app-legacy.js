...
// Ruta para obtener historial de permisos
app.get("/api/permission-history", async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const empresa = req.query.empresa || null;
...
app.get("/api/permissions", async (req, res) => {
  try {
    ...
    const permissions = await db.getPermissionsByEmployeeId(empleado_id);
...
// Ruta para crear una nueva solicitud de permiso
app.post("/api/permissions", async (req, res) => {
  try {
    ...
    const newPermissionId = await db.insertPermissionRequest(
      empleado_id,
      descripcion,
      con_goce_sueldo,
      fecha_inicio,
...
// Ruta para actualizar estado de permiso
app.put("/api/permissions/:permissionId", async (req, res) => {
  try {
    ...
    await db.updatePermissionStatus(permissionId, estado);
...
// Ruta para eliminar solicitud de permiso
app.delete("/api/permissions/:id", async (req, res) => {
  try {
    const permissionId = req.params.id;
    await db.deletePermissionRequest(permissionId);
